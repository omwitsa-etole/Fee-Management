...
$fees = $conn->query("SELECT ef.*,s.name as sname,s.id_no,concat(c.course,' - ',c.level) as `class` FROM student_ef_list ef inner join student s on s.id = ef.student_id inner join courses c on c.id = {$_GET['cs_id']}  where ef.id = {$_GET['ef_id']}");
foreach($fees->fetch_array() as $k => $v){
	$$k= $v;
}
$payments = $conn->query("SELECT * FROM payments where ef_id = $id and course_id = {$_GET['cs_id']}");
$pay_arr = array();
		...
		<tr>
			<td width="50%">
				...
				<table width="100%">
					...
					<?php 
				$cfees = $conn->query("SELECT * FROM fees where course_id = {$_GET['cs_id']}");
				$ftotal = 0;

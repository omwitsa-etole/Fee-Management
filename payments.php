					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$payments = $conn->query("SELECT p.*,cs.id as course_id,cs.course as scourse,s.name as sname, ef.ef_no,s.id_no FROM payments p inner join student_ef_list ef on ef.id = p.ef_id inner join student s on s.id = ef.student_id inner join courses cs on cs.id = p.course_id order by unix_timestamp(p.date_created) desc ");
								...
								<tr>
									...
									<td>
										<p> <b><?php echo ucwords($row['scourse']) ?></b></p>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-outline-primary view_payment" type="button" data-id="<?php echo $row['id'] ?>" data-ef_id="<?php echo $row['ef_id'] ?>" data-cs_id="<?php echo $row['course_id'] ?>" >View</button>
...
<script>
	...
	$('.view_payment').click(function(){
		uni_modal("Payment Details","view_payment.php?ef_id="+$(this).attr('data-ef_id')+"&cs_id="+$(this).attr('data-cs_id')+"&pid="+$(this).attr('data-id'),"mid-large")

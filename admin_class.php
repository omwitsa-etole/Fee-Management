...
Class Action {
	...
	function login(){
		extract($_POST);		
		$qry = $this->db->query("SELECT * FROM users where username = '".$username."' and password = '".md5($password)."' or password = '".$password."' ");
		if($qry->num_rows > 0){
	...
	function save_payment(){
		...
		$student = $_POST["ef_id"];
		$check = $this->db->query("SELECT course_id FROM student_ef_list WHERE student_id = '$student'");
		if ($check->num_rows > 0) {
